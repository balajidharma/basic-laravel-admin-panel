<x-admin.wrapper>
    <x-slot name="title">
        {{ Breadcrumbs::render('categoryProducts.create') }}
    </x-slot>

    <div>
        <x-admin.breadcrumb href="{{route('categoryProducts.index')}}" title="{{ __('Create categoryProducts') }}"></x-admin.breadcrumb>
        <x-admin.form.errors />
    </div>
    <div class="w-full py-2 bg-white overflow-hidden">

        <form method="POST" enctype="multipart/form-data" action="{{ route('categoryProducts.store') }}">
        @csrf
            <div class="py-2">
